...
上面的测试代码中`addRequireNew()`方法声明了`@Transactional`注解并且设置传播行为为`Propagation.REQUIRED`，根据之前的分析，该方法将会被Spring代理，运行在事务之中，而该方法内部调用的`user1Service.addRequireNew(user1)`方法也声明了`@Transactional`注解并且设置传播行为为`Propagation.REQUIRES_NEW`，根据之前的分析，`addRequireNew()`方法会被[TransactionAspectSupport]代理执行，当[TransactionAspectSupport]开启事务之后调用`invocation.proceedWithInvocation()`执行被代理方法，也就是这里的`addRequireNew()`方法，方法内调用`user1Service.addRequireNew(user1)`，由于该方法也声明了`@Transactional`注解，所以运行时同样会被[TransactionAspectSupport]代理执行，现在以事务开启之后，`user1Service.addRequireNew(user1)`方法执行之前的时间点开始分析`Propagation.REQUIRES_NEW`的实现，此时`user1Service.addRequireNew(user1)`的执行同样会执行下面的代码：
```java
TransactionInfo txInfo = createTransactionIfNecessary(tm, txAttr, joinpointIdentification);
Object retVal = null;
try {
