...
- PROPAGATION_REQUIRES_NEW：新建事务，如果当前存在事务，把当前事务挂起，子事务和被挂起事务互不影响，子事务之间也互不影响
- PROPAGATION_NOT_SUPPORTED：以非事务方式执行操作，如果当前存在事务，就把当前事务挂起
- PROPAGATION_NEVER：以非事务方式执行，如果当前存在事务，则抛出异常
- PROPAGATION_NESTED：如果当前存在事务，则在嵌套事务内执行，如果当前没有事务，则新建一个，嵌套事务回滚不会影响其他嵌套事务和被挂起事务，被挂起事务回滚会导致所有嵌套事务回滚
...
`matches`方法以tas是否为空及tas`getTransactionAttribute()`方法结果是否为空作为返回值，所以下面需要看[AnnotationTransactionAttributeSource]的实现，下面是[AnnotationTransactionAttributeSource]的继承结构：
...
接口中用到的[TransactionAttribute]的作用是返回方法上的事务属性，代码：
```java
...
[AnnotationTransactionAttributeSource]内部定义了多个[TransactionAnnotationParser]的实现，获取[TransactionAttribute]的过程实际上就是这些[TransactionAnnotationParser]实现的，以Spring的[Transactional]注解为例，实现是[SpringTransactionAnnotationParser]类，[SpringTransactionAnnotationParser]解析注解属性并将这些属性保存到[RuleBasedTransactionAttribute]实例中并返回，代码：
```java
@SuppressWarnings("serial")
...
如果看过笔记[从容器获取Bean](从容器获取Bean.md)和[如何实现AOP](如何实现AOP.md)，那对上面的接口不会陌生，所以下面直接看实现类，首先是[TransactionAspectSupport]，该类实现了事务代理执行过程中的所有逻辑，代码：
```java
public abstract class TransactionAspectSupport implements BeanFactoryAware, InitializingBean {
	...
	protected Object invokeWithinTransaction(Method method, @Nullable Class<?> targetClass,
		...
		if (txAttr == null || !(tm instanceof CallbackPreferringPlatformTransactionManager)) {
            ...
            // 如果txAttr不为空则会开启事务，否则还是会创建TransactionInfo对象但是不开启事务，TransactionInfo的TransactionStatus为空
...
[PlatformTransactionManager]中用到的[TransactionStatus]表示的是事务对象，能够获取事务信息并对事务进行操作，[TransactionStatus]接口还继承了[SavepointManager]接口，使得[SavepointManager]接口定义了使用和管理`savepoint`的方法，利用`savepoint`可以在回滚时指定回滚到特定的点，即可以实现回滚部分事务，内嵌事务就是通过`savepoint`实现的
...
最后是[DataSourceTransactionManager]类，实现了[AbstractPlatformTransactionManager]定义的抽象方法，主要是实现[DataSource]和[Connection]对象的操作，代码：
```java
@SuppressWarnings("serial")
...
下面分析事务执行过程的逻辑，先以最简单的只调用一个方法，事务传播行为为`Propagation.REQUIRED`的情况，在一个方法上添加注解`Transactional`（或在类上添加注解），则该方法所在类将会被代理，如：
```java
@Transactional(propagation = Propagation.REQUIRED)
...
之后`getTransaction()`方法返回[DefaultTransactionStatus]，回到`createTransactionIfNecessary()`方法，获取到[TransactionStatus]后调用`prepareTransactionInfo(tm, txAttr, joinpointIdentification, status)`返回，代码：
```java
protected TransactionInfo prepareTransactionInfo(@Nullable PlatformTransactionManager tm,
		@Nullable TransactionAttribute txAttr, String joinpointIdentification,
		@Nullable TransactionStatus status) {
...
`createTransactionIfNecessary()`方法执行完成后，`invokeWithinTransaction()`方法调用`invocation.proceedWithInvocation()`执行被代理方法，代码：
```java
TransactionInfo txInfo = createTransactionIfNecessary(tm, txAttr, joinpointIdentification);
Object retVal = null;
try {
...
如果发生异常则执行catch代码块中的`completeTransactionAfterThrowing`方法，代码：
```java
protected void completeTransactionAfterThrowing(@Nullable TransactionInfo txInfo, Throwable ex) {
	if (txInfo != null && txInfo.getTransactionStatus() != null) {
		if (logger.isTraceEnabled()) {
...
如果发生的异常需要回滚，则执行`txInfo.getTransactionManager().rollback(txInfo.getTransactionStatus())`，该方法实现在[AbstractPlatformTransactionManager]类中，代码：
```java
@Override
public final void rollback(TransactionStatus status) throws TransactionException {
	// 如果事务已经完成，此时回滚报错
...
对于新建的事务，执行了`doRollback()`方法，该方法为抽象方法，[DataSourceTransactionManager]的实现如下：
```java
@Override
protected void doRollback(DefaultTransactionStatus status) {
	DataSourceTransactionObject txObject = (DataSourceTransactionObject) status.getTransaction();
...
可以看到，回滚就是简单的执行[Connection]对象上的`rollback`方法，回到`processRollback()`方法，回滚之后执行`cleanupAfterCompletion()`方法，代码：
```java
private void cleanupAfterCompletion(DefaultTransactionStatus status) {
	status.setCompleted();
	if (status.isNewSynchronization()) {
...
对于commit也是类似的过程，以上是最简单的只有一个事务的情况下事务的执行过程，对于其他传播行为的事务也是这个执行过程，总结一下就是通过AOP代理方法的执行，在执行被代理方法之前获取获取[Connection]对象并保存起来，之后执行被代理方法，如果执行出错通过[Connection]对象进行rollback，否则执行commit，另外对于例子中被代理方法的数据库操作实际上是通过[JdbcTemplate]实现的，而[JdbcTemplate]中会获取[Connection]对象进行数据库操作，但是Spring执行事务时已经创建好了[Connection]对象了，所以spring-jdbc和spring-tx这两个模块之间肯定有联系，这一部分可以看笔记[JdbcTemplate的实现](JdbcTemplate的实现.md)中如果从[DataSourceUtils]类中获取[Connection]的

下面再分析复杂点的情况，已存在事务时子事务是如何执行的，先看子事务传播行为为`Propagation.REQUIRES_NEW`的情况，测试代码：
```java
@Transactional(propagation = Propagation.REQUIRED)
public void addRequireNew() {
	User1 user = new User1();
	user.setName("张三");
	user1Service.addRequireNew(user);
}
...
上面的测试代码中`addRequireNew()`方法声明了`@Transactional`注解并且设置传播行为为`Propagation.REQUIRED`，根据之前的分析，该方法将会被Spring代理，运行在事务之中，而该方法内部调用的`user1Service.addRequireNew(user)`方法也声明了`@Transactional`注解并且设置传播行为为`Propagation.REQUIRES_NEW`，根据之前的分析，`addRequireNew()`方法会被[TransactionAspectSupport]代理执行，当[TransactionAspectSupport]开启事务之后调用`invocation.proceedWithInvocation()`执行被代理方法，也就是这里的`addRequireNew()`方法，方法内调用`user1Service.addRequireNew(user)`，由于该方法也声明了`@Transactional`注解，所以运行时同样会被[TransactionAspectSupport]代理执行，现在以事务开启之后，`user1Service.addRequireNew(user)`方法执行之前的时间点开始分析`Propagation.REQUIRES_NEW`的实现，此时`user1Service.addRequireNew(user)`的执行同样会执行下面的代码：
```java
TransactionInfo txInfo = createTransactionIfNecessary(tm, txAttr, joinpointIdentification);
Object retVal = null;
try {
...
调用了`createTransactionIfNecessary(tm, txAttr, joinpointIdentification)`方法尝试创建事务，代码：
```java
@SuppressWarnings("serial")
protected TransactionInfo createTransactionIfNecessary(@Nullable PlatformTransactionManager tm,
		@Nullable TransactionAttribute txAttr, final String joinpointIdentification) {
...
// AbstractPlatformTransactionManager的getTransaction方法实现
@Override
// 参数TransactionDefinition定义了事务的属性，包括传播行为、隔离级别、超时时间和是否只读，对于TransactionAspectSupport的createTransactionIfNecessary
// 方法，传入的参数就是AnnotationTransactionAttributeSource对象根据Transactional注解创建的RuleBasedTransactionAttribute对象
public final TransactionStatus getTransaction(@Nullable TransactionDefinition definition) throws TransactionException {
...
和没有事务时的执行过程不同的地方在于`isExistingTransaction()`方法的判断：
```java
Object transaction = doGetTransaction();
...
// DataSourceTransactionManagerd实现
@Override
protected boolean isExistingTransaction(Object transaction) {
	DataSourceTransactionObject txObject = (DataSourceTransactionObject) transaction;
	return (txObject.hasConnectionHolder() && txObject.getConnectionHolder().isTransactionActive());
...
`isExistingTransaction()`方法的参数从`doGetTransaction()`方法获得，代码：
```java
// DataSourceTransactionManagerd实现
@Override
protected Object doGetTransaction() {
...
这里获得的[ConnectionHolder]实际上是执行`addRequireNew()`前开启事务时创建的，所以上面`isExistingTransaction()`将返回true，此时进入`handleExistingTransaction()`方法，代码：
```java
private TransactionStatus handleExistingTransaction(
		TransactionDefinition definition, Object transaction, boolean debugEnabled)
		throws TransactionException {
...
对于传播行为为`PROPAGATION_REQUIRES_NEW`的事务，执行的是下面的代码：
```java
if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_REQUIRES_NEW) {
	if (debugEnabled) {
		logger.debug("Suspending current transaction, creating new transaction with name [" +
...
挂起操作就是将已存在的事务的相关信息保存到[SuspendedResourcesHolder]对象中，同时调用`doSuspend()`方法传入`getTransaction()`方法中调用`doGetTransaction()`方法为当前正在创建的事务创建的[DataSourceTransactionObject]对象，`doSuspend()`方法是个抽象方法，下面是[DataSourceTransactionManager]的实现，代码：
```java
...
`doSuspend()`方法将[DataSourceTransactionObject]的[ConnectionHolder]清空并且移除已存在事务和[DataSource]的映射关系，这样当前线程中已存在事务的信息已经清除了，`suspend()`执行之后执行`newTransactionStatus`方法创建[DefaultTransactionStatus]，代码：
```java
boolean newSynchronization = (getTransactionSynchronization() != SYNCHRONIZATION_NEVER);
DefaultTransactionStatus status = newTransactionStatus(
		definition, transaction, true, newSynchronization, debugEnabled, suspendedResources);
...
创建[DefaultTransactionStatus]之后调用`doBegin()`方法开启事务，该方法是抽象方法，下面是[DataSourceTransactionManager]的实现，代码：
```java
@Override
protected void doBegin(Object transaction, TransactionDefinition definition) {
	DataSourceTransactionObject txObject = (DataSourceTransactionObject) transaction;
...
[DataSourceTransactionObject]对象的[ConnectionHolder]已经被清空了，所以会重新获取一个[Connection]对象，这就是创建子事务的本质，`doBegin()`方法之后执行`prepareSynchronization()`方法，代码：
```java
protected void prepareSynchronization(DefaultTransactionStatus status, TransactionDefinition definition) {
	// newSynchronization为true表示需要将当前事务配置保存到TransactionSynchronizationManager中
	if (status.isNewSynchronization()) {
...
`prepareSynchronization()`方法相当于将新创建的事务保存到线程中，`handleExistingTransaction()`方法执行完成后回到`invokeWithinTransaction()`方法，和只有一个事务时一样的执行过程，区别在于子事务提交的处理，这发生在`commitTransactionAfterReturning()`方法中，代码：
```java
protected void commitTransactionAfterReturning(@Nullable TransactionInfo txInfo) {
	if (txInfo != null && txInfo.getTransactionStatus() != null) {
		if (logger.isTraceEnabled()) {
...
// AbstractPlatformTransactionManager实现
@Override
public final void commit(TransactionStatus status) throws TransactionException {
	// 如果事务已经完成则抛出异常
	if (status.isCompleted()) {
...
对于创建了子事务的情况，`processCommit()`方法执行的是下面的代码
```java
if (status.isNewTransaction()) {
	if (status.isDebug()) {
		logger.debug("Initiating transaction commit");
...
最后finally语句中的`cleanupAfterCompletion()`方法完成清空数据库连接和恢复被挂起事务的操作，代码：
```java
private void cleanupAfterCompletion(DefaultTransactionStatus status) {
	// 标记当前事务已完成
	status.setCompleted();
...
上面是`PROPAGATION_REQUIRES_NEW`的实现过程，`PROPAGATION_REQUIRES_NEW`的传播行为创建的子事务之间和已存在事务之间互不影响实际上就是通过每次子事务执行时挂起已存在事务并自己重新获取[Connection]实现的，另外一个传播行为`PROPAGATION_NESTED`的实现，和`PROPAGATION_REQUIRES_NEW`不同，默认使用`savepoint`实现嵌套事务，代码：
```java
private TransactionStatus handleExistingTransaction(
		TransactionDefinition definition, Object transaction, boolean debugEnabled)
		throws TransactionException {
...
对于嵌套事务，不是创建一个新的[Connection]，而是创建`savepoint`，代码：
```java
public void createAndHoldSavepoint() throws TransactionException {
	// getSavepointManager返回的实际上是DataSourceTransactionObject
	setSavepoint(getSavepointManager().createSavepoint());
}

// DataSourceTransactionObject实现
@Override
...
当嵌套事务出错需要回滚时执行的`processRollback()`方法代码：
```java
private void processRollback(DefaultTransactionStatus status, boolean unexpected) {
	try {
		boolean unexpectedRollback = unexpected;
...
上面判断了是否存在保存点，如果存在则回滚到保存点而不是调用[Connection]的`rollback`方法，这就是`PROPAGATION_NESTED`的实现过程，对于其他类型的传播行为也大同小异，源码注释也都做了分析，这里不再赘述

[BookDao]: aaa
[TxNamespaceHandler]: aaa
[BeanDefinition]: aaa
[NamespaceHandler]: aaa
[PointcutAdvisor]: aaa
[MethodMatcher]: aaa
[TransactionAttributeSourcePointcut]: aaa
[TransactionAnnotationParser]: aaa
[Transactional]: aaa
[SpringTransactionAnnotationParser]: aaa
[Advice]: aaa
[BeanFactory]: aaa
[PlatformTransactionManager]: aaa
[AbstractPlatformTransactionManager]: aaa
[AbstractFallbackTransactionAttributeSource]: aaa
[InfrastructureAdvisorAutoProxyCreator]: aaa
[AnnotationDrivenBeanDefinitionParser]: aaa
[AnnotationTransactionAttributeSource]: aaa
[TransactionInterceptor]: aaa
[DefaultTransactionStatus]: aaa
[ConnectionHolder]: aaa
[TransactionSynchronizationManager]: aaa
[ThreadLocal]: aaa
