...
[PlatformTransactionManager]中用到的[TransactionStatus]表示的是事务对象，能够获取事务信息并对事务进行操作，[TransactionStatus]接口还继承了[SavepointManager]接口，使得[SavepointManager]接口定义了使用和管理`savepoint`的方法，`savepoint`是JDBC提供的功能，能够在回滚时指定回滚到特定的`savepoint`
