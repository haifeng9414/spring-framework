...
对于commit也是类似的过程，以上是最简单的只有一个事务的情况下事务的执行过程，总结一下就是通过AOP代理方法的执行，在执行被代理方法之前获取获取[Connection]对象并保存起来，之后执行被代理方法，如果执行出错通过[Connection]对象进行rollback，否则执行commit，另外对于例子中被代理方法的数据库操作实际上是通过[JdbcTemplate]实现的，而[JdbcTemplate]中会获取[Connection]对象进行数据库操作，但是Spring执行事务时已经创建好了[Connection]对象了，所以spring-jdbc和spring-tx这两个模块之间肯定有联系，这一部分可以看笔记[JdbcTemplate的实现](JdbcTemplate的实现.md)中如果从[DataSourceUtils]类中获取[Connection]的
