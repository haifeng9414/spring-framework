...
所以[DispatcherServlet]的分发过程简单来说就是从[HandlerMapping]中获取并执行请求对应的[HandlerExecutionChain]，并用[ViewResolver]解析[HandlerExecutionChain]返回的[ModelAndView]（这一过程可以看笔记[InternalResourceViewResolver的实现](InternalResourceViewResolver的实现.md)），最后通过解析到的[View]对象返回视图
