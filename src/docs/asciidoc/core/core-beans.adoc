...
exactly one id. Conventionally these names are alphanumeric ('myBeanA',
...
with an id of `myBeanA`, invoking `getBean("myBeanA")` on the container returns the
product of the `FactoryBean`; whereas, invoking `getBean("&myBeanA")` returns the
